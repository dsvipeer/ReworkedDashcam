...
Citizen.CreateThread(function()
    while true do
        if IsControlJustPressed(1, DashcamConfig.Keybind) and IsPedInAnyVehicle(GetPlayerPed(PlayerId()), false) then
            ...
            if vehicleClass == 18 then
                if dashcamActive then
                    ...
                    TriggerServerEvent("Server:SoundToRadius", netID, 2, "dash", 0.05)
                end
            end
        end
        ...
        if dashcamActive then
            ...
            SetCamRot(cameraHandle, vehRot.x, vehRot.y, vehRot.z, 2)
        end
        Citizen.Wait(0)
    end
end)
...
function EnableDash()
    ...
    if DashcamConfig.RestrictVehicles then
        if CheckVehicleRestriction() then
            ...
            dashcamActive = true
            if DashcamConfig.AutomaticRecording then
                StartRecording(1)
            end
        end
    else
